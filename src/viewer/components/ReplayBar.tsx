  ...
  return (
    <section className="border-b border-ta-grey-400">
      <div className="flex items-center gap-4 px-6 py-3">
        ...
        <Button onClick={replay.toggle} aria-label={replay.playing ? "pause" : "play"}>
      ...
      <div className="px-6 pb-4">
        <div className="relative h-16 w-full border border-ta-grey-400 bg-ta-grey-500">
          {trace.generations.map((gen) => {
            ...
            offset += gen.metrics.latency;
            const width = (gen.metrics.latency / total) * 100;
            const ttftShare = Math.min(gen.metrics.timeToFirstToken / gen.metrics.latency, 1);
            ...
            return (
              <button
                ...
                onClick={() => {
                  replay.pause();
                  replay.seekToGeneration(gen.index);
                  onSelect(gen.index);
                }}
                aria-pressed={selected}
                title={`[${gen.index}] ${formatSeconds(gen.metrics.latency)}, ttft ${formatSeconds(
                  gen.metrics.timeToFirstToken,
